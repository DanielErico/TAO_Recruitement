/**
 * Complete list of Nigeria's 36 States + FCT and all their LGAs.
 * Source: Nigerian official administrative divisions (774 LGAs total).
 */
...
export const NIGERIA_STATES_LGAS: NigeriaState[] = [
  ...
  },
  {
    ...
    lgas: [
      ...
      "Oluyole", "Ona Ara", "Orelope", "Orire", "Oyo East", "Oyo West",
...
/** Lookup map: state name → array of LGAs */
export const LGA_BY_STATE: Record<string, string[]> = Object.fromEntries(
  NIGERIA_STATES_LGAS.map(({ state, lgas }) => [state, lgas])
);

/** Sorted list of all state names */
export const STATE_NAMES = NIGERIA_STATES_LGAS.map(({ state }) => state);
